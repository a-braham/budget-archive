...
from .data import CREATE_MUTATION, UPDATE_MUTATION, ONE_ACCOUNT, ALL_ACCOUNTS


class AccountTestCase(BaseTest):
    def test_create_account_user_is_anonymous(self):
        self.snapshot(
            request_string=CREATE_MUTATION,
            variables={
                "name": "Card",
                "institution": "Bank",
                "type": "DEBIT",
    ...
    def test_update_account(self):
        self.snapshot(
            request_string=UPDATE_MUTATION,
            context={"user": self.user},
            variables={
                "id": self.account.id,
                "name": "Card",
